...
const { version } = pkg;
const baseDir = `/tmp/csslit-rust-transformer-${version}`;
const bindingEntry = `${baseDir}/node_modules/@csslit/rust-transformer-wasm32-wasi/csslit-transformer.wasi.cjs`;

if (!fs.existsSync(bindingEntry)) {
  fs.rmSync(baseDir, { recursive: true, force: true });
  fs.mkdirSync(baseDir, { recursive: true });
  const bindingPkg = `@csslit/rust-transformer-wasm32-wasi@${version}`;
